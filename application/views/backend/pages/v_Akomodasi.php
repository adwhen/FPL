    ...
    <section class="content">
      ...
      <div class="box">
        <div class="box-header">
          <center>
            <h3 class="box-title">Pelayanan Akomdasi SPPD</h3>
          </center>
        </div>
        <hr>
        <div class="box-header">
          <a class="btn btn-success" href="<?= base_url('index.php/Accomodation/form') ?>">Ajukan Akomodasi</a>
          <?php if ($this->session->userdata('status') == 1) { ?><a class="btn btn-warning" href="<?= base_url('index.php/Accomodation/report') ?>">Laporan</a><?php } ?>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
              ...
              foreach ($data->result() as $dt) { ?>
                <tr>
                  ...
                  <td>
                    ...
                    <?= $dt->ACARA_A ?></td>
                  <td><?= $dt->UNIT_KERJA_A ?></td>
                  <td><?= $dt->NOMOR_SPPD ?></td>
                  <td>
                    <?php if ($this->session->userdata('status') == 0) { ?>
                      <a href="<?= base_url('index.php/Accomodation/form/' . $dt->IDX_A) ?>" class="btn btn-warning"><i class="fa fa-edit"> Edit</i></a>
                      <a target="_BLANK" href="<?= base_url('index.php/Accomodation/REC/' . $dt->IDX_A) ?>" class="btn btn-primary"><i class="fa fa-file-pdf-o"> PDF</i></a>
                    <?php } else { ?>
                      <?php if ($dt->PROCESS == "KIRIM") { ?>
                        <a href="#" onclick="pop_up('<?= $dt->IDX_A ?>')" class="btn btn-primary" data-toggle="modal" data-target="#modal-default"><i class="fa fa-check"></i></a>
                      <?php } else { ?>
                        <a target="_BLANK" href="<?= base_url('index.php/Accomodation/REC/' . $dt->IDX_A) ?>" class="btn btn-success"><i class="fa fa-file-pdf-o"> PDF</i></a>
                      <?php } ?>
                    <?php } ?>
...
<script type="text/javascript">
  function pop_up(IDX) {
    var url = '<?= base_url('index.php/Accomodation/REC_Approve/') ?>'
  ...
  function save_pel(idx) {
    ...
    $.post('<?= base_url('index.php/Accomodation/approval/') ?>', {
        IDX: idx,
        KOMENTAR: komentar,
        STATUS: status
      },
      function(data) {
        if (data.trim() == "OK") {
          ...
          window.open('<?= base_url() ?>index.php/Accomodation', '_self');
