      ...
      <section class="content">
        ...
        <div class="box">
            <div class="box-header">
              <center><h3 class="box-title">Pelayanan Umum</h3></center>
            </div>
            <hr>
            <div class="box-header">
              <a class="btn btn-success" href="<?=base_url('index.php/Pelayanan_umum/form')?>">Ajukan Permohonan</a>  
            </div>
            <!-- /.box-header --> 
            <div class="box-body">
              ...
              <?php $no=1; foreach($data->result() as $dt){ ?>
                <tr>
                  ...
                  <td>
                    ...
                    <?=$dt->PERMOHONAN_UMUM?></td>
                  <td><?=$dt->TUJUAN_UMUM?></td>
                  <td><?=$dt->DATE_UMUM."-".$dt->TIME_UMUM?></td>
                  <td>
                    <?php if($this->session->userdata('status')==0){ ?>
                      <a href="<?=base_url('index.php/Pelayanan_umum/form/'.$dt->IDX_UMUM)?>" class="btn btn-warning"><i class="fa fa-edit"> Edit</i></a>
                      <a target="_BLANK" href="<?=base_url('index.php/Pelayanan_umum/REC/'.$dt->IDX_UMUM)?>" class="btn btn-primary"><i class="fa fa-file-pdf-o"> PDF</i></a>
                    <?php }else{ ?>
                      <?php if($dt->PROCESS=="KIRIM"){ ?>
                      <a href="#" onclick="pop_up('<?=$dt->IDX_UMUM?>')" class="btn btn-primary" data-toggle="modal" data-target="#modal-default"><i class="fa fa-check"></i></a>
                      <?php }else{ ?>
                              <a target="_BLANK" href="<?=base_url('index.php/Pelayanan_umum/REC/'.$dt->IDX_UMUM)?>" class="btn btn-success"><i class="fa fa-file-pdf-o"> PDF</i></a>
                      <?php } ?>
                    <?php } ?>
...
<script type="text/javascript">
  function pop_up(IDX){
    var url = '<?=base_url('index.php/Pelayanan_umum/REC_Approve/')?>'
  ...
  function save_pel(idx){
    ...
    $.post('<?=base_url('index.php/pelayanan_umum/approval/')?>',{IDX:idx,KOMENTAR:komentar,STATUS:status},
      function(data){
        if(data.trim()=="OK"){
          Swal.fire({
            icon: 'success',
          ...
          window.open('<?=base_url()?>index.php/Pelayanan_umum','_self');
